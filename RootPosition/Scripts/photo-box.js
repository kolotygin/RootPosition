...
RootPosition.PhotoBox = function (options) {
	...
	this._ie6 = (jQuery.browser.msie && parseFloat(jQuery.browser.version) < 7);
...
RootPosition.PhotoBox.prototype = {
	...
	_displayLayout: function () {
		...
		jQuery(document).bind(jQuery.browser.mozilla ? "keypress" : "keydown", this._onKeyDownFunction);
	...
	_hideLayout: function () {
		...
		HtmlExtension.RemoveElement(this._container);
		HtmlExtension.RemoveElement(this._overlay);
		...
		jQuery(document).unbind(jQuery.browser.mozilla ? "keypress" : "keydown", this._onKeyDownFunction);
		this._releseFocus();
	},
